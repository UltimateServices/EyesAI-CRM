...
export default function DiscoverClientsPage() {
  ...
  // Filter for Discover package clients who have completed onboarding
  const discoverClients = companies.filter((company) =>
    company.plan?.toLowerCase() === 'discover' &&
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      ...
      for (const client of discoverClients) {
        try {
    ...
    if (discoverClients.length > 0) {
      fetchProgress();
    }
  }, [discoverClients.length]);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 mt-1">
            Manage monthly deliverables for Discover package clients ($39/month)
          </p>
        </div>
        <Badge variant="outline" className="text-lg px-4 py-2">
          {discoverClients.length} {discoverClients.length === 1 ? 'Client' : 'Clients'}
        </Badge>
      </div>

      {/* Client List */}
      {discoverClients.length === 0 ? (
        <Card className="p-12 text-center">
          <Users className="w-16 h-16 text-slate-300 mx-auto mb-4" />
          <h3 className="text-xl font-semibold text-slate-900 mb-2">
            No Discover Clients Yet
          </h3>
          <p className="text-slate-600 mb-4">
            Clients will appear here once they complete onboarding with a Discover package.
          </p>
                ...
                <Card className="overflow-hidden hover:shadow-lg transition-all cursor-pointer group">
                  ...
                  <div className="p-6 bg-gradient-to-br from-white to-slate-50">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <Badge className="bg-white text-blue-600 border border-blue-600 flex items-center gap-2">
                        <img
                          src="/discover-icon.png"
                          alt="Discover"
                          className="w-5 h-4"
