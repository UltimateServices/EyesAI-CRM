...
// POST /api/onboarding/send-welcome-email - Send email only (no completion)
...
async function sendWelcomeEmail(emailData: EmailData): Promise<boolean> {
  try {
    ...
    const profileResponse = await fetch('https://a.klaviyo.com/api/profiles/', {
      ...
      body: JSON.stringify({
        data: {
          ...
          attributes: {
            email: emailData.clientEmail,
            first_name: emailData.ownerName,
            properties: {
              business_name: emailData.businessName,
              business_slug: emailData.businessSlug,
              ...
              temp_password: emailData.tempPassword,
            },
          },
        },
      }),
          ...
          attributes: {
            ...
            metric: {
              data: {
                type: 'metric',
                attributes: {
                  name: 'Welcome Email Requested',
                },
              },
            },
            properties: {
              business_name: emailData.businessName,
              owner_name: emailData.ownerName,
              business_slug: emailData.businessSlug,
    ...
    if (!eventResponse.ok) {
      ...
      throw new Error('Failed to trigger Klaviyo event');
    }

    console.log('✅ Klaviyo event triggered: Welcome Email Requested');
